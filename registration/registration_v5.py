...
def MSE_nonEmpty(ip1,ip2):
	...
	return sumOfSquares( minus(ip1,ip2) ) / maskArea( maskIntersection(ip1,ip2) )
...
def VOP():
	""" The VOP (= Volume Overlap Percentage) is defined as VOPj  =  2 * V( intersection(Aj,Bj) ) / ( V(Aj) + V(Bj) ) with V() the volume function, label j, and labeled pixels in image A and B. """
	return 0
